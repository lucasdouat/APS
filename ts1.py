...
signals = [x2, x3, x4, x5, x6]
labels = ['x2', 'x3', 'x4', 'x5', 'x6']

# Función para calcular producto interno y coseno del ángulo
def analyze_orthogonality(x1, signals):
    results = []
    norm_x1 = np.linalg.norm(x1)
    for i, xi in enumerate(signals):
        dot_product = np.dot(x1, xi)
        norm_xi = np.linalg.norm(xi)
        cos_theta = dot_product / (norm_x1 * norm_xi)
        results.append((labels[i], dot_product, cos_theta))
    return results

# Calcular ortogonalidad
results = analyze_orthogonality(x1, signals)

# Mostrar resultados
for label, dot, cos in results:
    print(f"Producto interno entre x1 y {label}: {dot:.2f}, Coseno del ángulo: {cos:.4f}")
